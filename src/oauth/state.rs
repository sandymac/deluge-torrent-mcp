...
pub(crate) const REFRESH_TOKEN_TTL: Duration = Duration::from_secs(24 * 60 * 60);
...
pub(crate) struct RefreshInfo {
    ...
    pub(crate) superseded_at: Option<Instant>,
}
...
impl OAuthState {
    ...
    pub(crate) async fn revoke_access_token(&self, token: &str) {
    ...
    pub(crate) async fn remove_refresh_token(&self, token: &str) {
    ...
    pub(crate) async fn mark_refresh_superseded(&self, token: &str, now: Instant) {
        let mut tokens = self.refresh_tokens.lock().await;
