...
- Reading files from the Deluge server's filesystem (when using `add_torrent` with an absolute path to a `.torrent` file on the server)
...
Even with these safety gates enabled, an LLM can still make significant changes to your torrent library and, depending on configuration, access files on the server.

## Important Risks

| Area | Notes |
|------|-------|
| **File access via `add_torrent`** | When a server-side file path is provided, the server can read any file the Deluge process is allowed to read. Paths containing `..` are rejected, but this is only a defense-in-depth measure. |
...
- Be especially careful with the "absolute file path on the Deluge server" input method for `add_torrent`.
